import { ApplicationCommandPermissionData, Client, Collection, CommandInteraction, Guild, GuildResolvable, Interaction, Snowflake } from "discord.js";
import { Command } from "./Command";
import { CommandRegisterer, GlobalCommandRegisterer, GuildCommandRegisterer } from "./Registerer";

export class CommandManager {
    private commandCollection: Collection<string, Command> = new Collection();
    private registerer: CommandRegisterer;
    constructor(public readonly client: Client, public readonly isGlobal: boolean, commands: Command[]) {
        this.commands = commands;
        if (isGlobal) {
            this.registerer = new GlobalCommandRegisterer(client, this.commandDefinitions);
        } else {
            this.registerer = new GuildCommandRegisterer(client, this.commandDefinitions);
        }
        ...
        for (const commandName in this.commandCollection.keys()) {
            await Promise.all(
                this.client.guilds.cache.map(async guild => {
                    const permission = this.commandCollection.get(commandName)?.permission;
    ...
    public async setPermission(guildId: Snowflake, commandName: string, permissions: ApplicationCommandPermissionData[]) {
    ...
    private async interactionCreateHandler(interaction: CommandInteraction<'cached'>) {
